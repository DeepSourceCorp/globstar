...
var (
	// Flag to control running only the DFG tests
	dfgOnly = flag.Bool(
		"dfg-only",
		false, // Default to false: run both basic and DFG
		"Run only the DFG SQL injection tests",
	)
)
...
func TestSQLInjection(t *testing.T) {
	tests := []struct {
        ...
        findUser(taintedId); // Call with tainted data
    `,
			wantVuln: true,
		},
		{
	...
	for _, tt := range tests {
		...
		// --- Run Basic Test (if dfgOnly is false) ---
		if !*dfgOnly {
